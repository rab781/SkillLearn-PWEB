...
console.log('Quiz Questions JavaScript loaded successfully');
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM loaded, initializing quiz questions manager...');
    ...
    if (quizIdMeta) {
        ...
        console.log('Quiz ID found:', quiz_id);
    } else {
        console.error('Quiz ID meta tag not found');
    }
...
function showQuestionBuilder() {
    console.log('showQuestionBuilder called');
    
    const builderCard = document.getElementById('questionBuilderCard');
    if (builderCard) {
        console.log('Builder card found, showing...');
...
function addAnswerOption() {
    console.log('addAnswerOption called');
    ...
    console.log('Adding answer option:', optionLabel);
        ...
        <div class="answer-option-item" data-option="${optionLabel}">
            <div class="answer-option-header">
                ...
                <div class="form-group" style="flex: 1; margin-bottom: 0;">
                    <input type="text" name="pilihan_jawaban[]" class="answer-input" placeholder="Masukkan pilihan jawaban..." required>
                </div>
                <div class="correct-answer-toggle">
                    <div class="form-check">
                        <input type="radio" name="jawaban_benar" value="${optionLabel}" id="correct_${optionLabel}" class="form-check-input">
                        <label for="correct_${optionLabel}" class="form-check-label">Jawaban Benar</label>
                    </div>
                </div>
                ${optionIndex > 1 ? `
                    <button type="button" class="btn-remove-answer" onclick="removeAnswerOption('${optionLabel}')" style="margin-left: 0.75rem; background: #dc2626; color: white; border: none; padding: 0.5rem; border-radius: 4px; cursor: pointer;">
    ...
    console.log('Answer option added successfully, total count:', answerOptionCount);
}
...
function saveQuestion() {
    ...
    if (questionId) {
        ...
        url = `/admin/courses/quiz-questions/${questionId}`;
        method = 'PUT';
        formData.append('_method', 'PUT');
    } else {
        // Create new question
        url = `/admin/courses/quiz/${quiz_id}/questions`;
        method = 'POST';
    }
    ...
    const optionHtml = `
        <div class="answer-option-item ${isCorrect ? 'correct' : ''}" data-option="${optionLabel}">
            <div class="answer-option-header">
                ...
                <div class="form-group" style="flex: 1; margin-bottom: 0;">
                    <input type="text" name="pilihan_jawaban[]" class="answer-input" value="${optionValue}" placeholder="Masukkan pilihan jawaban..." required>
                </div>
                <div class="correct-answer-toggle">
                    <div class="form-check">
                        <input type="radio" name="jawaban_benar" value="${optionLabel}" id="correct_${optionLabel}" ${isCorrect ? 'checked' : ''} class="form-check-input">
                        <label for="correct_${optionLabel}" class="form-check-label">Jawaban Benar</label>
                    </div>
                </div>
                ${optionIndex > 1 ? `
                    <button type="button" class="btn-remove-answer" onclick="removeAnswerOption('${optionLabel}')" style="margin-left: 0.75rem;">
                        <i class="fas fa-times"></i>
                    </button>
                ` : ''}
            </div>
...
function deleteQuestion(questionId) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            const formData = new FormData();
            formData.append('_token', csrfToken);
            formData.append('_method', 'DELETE');            // Delete question
...
function updateQuestionOrder() {
    ...
    const formData = new FormData();
    formData.append('_token', csrfToken);
    formData.append('order_data', JSON.stringify(orderData));    // Update order
